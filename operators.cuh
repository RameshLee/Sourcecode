...
struct noise
{
	...
	void update_noise_parameters(solution *Solution)
	{
		...
		for (int i = 0; i<Var; i++)
			for (int j = 0; j < Var; j++)
			{
				if (max_dij == 0 || Solution[0].Problem.TravelCost[i][j] > max_dij)
					max_dij = Solution[0].Problem.TravelCost[i][j];
			}
...
struct removal
{
	...
	void shaw_removal(solution *TempSolution)
	{
		D_list_size = 0;
		...
		reset(req_list);
		update_relatedness(req, TempSolution);
		sort_relatedness(req_list);
	...
	void worst_removal(solution *TempSolution)
	{
		D_list_size = 0;

		reset(req_list);
		sort_worstcost(req_list, TempSolution);
	...
	void sort_worstcost(int *req_list, solution *TempSolution)
	{
		// initialization
		...
		Req_Sorting(req_list, SimpleSolution);
	...
	void Req_Sorting(int *list, simple_solution *SimpleSolution)
	{
		float *cost_effect = new float[n];

		for (int i = 0; i < n; i++)
			cost_effect[i] = SimpleSolution[0].request[i].cost_effect;
		...
		for (int i = 0; i < n; i++)
		{
			for (int j=i+1; j < n; j++)
			{
				...
				{
					temp_req_list = list[i];
					list[i] = list[j];
					list[j] = temp_req_list;
	...
	void operator_selection(int select, int original_intensity, solution *TempSolution)
	{
